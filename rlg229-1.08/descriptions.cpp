...
object* rolling(object_description obj_discrip){
// turn into an object

    object  *obj = new object();
	obj->name =	obj_discrip.name;
	obj->type = obj_discrip.type;
    obj->color = obj_discrip.color;
	obj->damage = obj_discrip.damage;
	obj->description = obj_discrip.description;
	
	int32_t hit = obj_discrip.hit.roll();
	int32_t dodge = obj_discrip.dodge.roll(); 
	int32_t defence = obj_discrip.defence.roll();
	int32_t weight = obj_discrip.weight.roll();
	int32_t speed = obj_discrip.speed.roll();
	int32_t attrubute = obj_discrip.attrubute.roll();
	int32_t value = obj_discrip.value.roll();
...
character_t* rolling(monster_description &mon_discrip){
// turn into an object
	   



    character_t* mon = ( character_t*)malloc(sizeof(mon));
	mon->name =	mon_discrip.name.c_str();
	mon->description = mon_discrip.description.c_str();	
    mon->color = mon_discrip.color;
	//mon->damage = (const dice_t*)&mon_discrip.damage;	
	int32_t hit = mon_discrip.hitpoints.roll();
...
object *obj= (object*)c_object; 
int32_t color = obj->color;
return color;
}
...
void generate_objects(dungeon_t *d){
  ...
  vector<object_description>* obj =(vector<object_description> *)d->object_descriptions;
  ...
  uint32_t room;
	...
	while(i < 10){
		generate_num = rand() % obj->size();
		object_description generate_object = (*obj)[generate_num];
		object *instance = rolling(generate_object);
		// place in random room
		 room = rand_range(0, d->num_rooms - 1);	
		 d->object[d->rooms[room].position[dim_y]][d->rooms[room].position[dim_x]] = instance;		
	}
...
void generate_monsters(dungeon_t *d , uint32_t game_turn){
  ...
  vector<monster_description> * mon =(vector<monster_description> *)d->monster_descriptions;
  
  int generate_num;
  int i = 0;
  uint32_t room;
  character_t* instance;
  
	while(i < 10){
	   	instance = (character_t*)malloc(sizeof (instance));
		generate_num = rand() % mon->size();
	    monster_description generate_mon = (*mon)[generate_num];	
		instance = (character_t *)rolling(generate_mon);
		// place in random room
		room = rand_range(0, d->num_rooms - 1);
		instance->symbol = 'A' + rand_range(0, 25) + ((rand() & 1) ? 32 : 0);
		
		instance->next_turn = game_turn;
		instance->alive = 1;
		instance->sequence_number = ++d->character_sequence_number;
		instance->pc = NULL;
        instance->npc = (npc_t*)malloc(sizeof (instance->npc));		
		instance->npc->characteristics = rand();
		instance->npc->have_seen_pc = 0;		 
		d->character[d->rooms[room].position[dim_y]][d->rooms[room].position[dim_x]] = instance;	
		heap_insert(&d->next_turn, instance);
	}
