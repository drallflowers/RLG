...
void generate_objects(dungeon_t *d){
  ...
  uint32_t room;
	...
	while(i < 10){
		...
		object_description generated_object = (*obj)[generate_num];
...
void generate_monsters(dungeon_t *d , uint32_t game_turn){
  ...
  uint32_t room;
	...
	while(i < 10){
		...
		mon_description generated_mon = (*mon)[generate_num];
		character_t *instance = rolling(generate_mon);
		// place in random room
		room = rand_range(0, d->num_rooms - 1);
